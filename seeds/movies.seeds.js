...
mongoose
  .connect('mongodb://localhost/Express-movies', { useNewUrlParser: true, useUnifiedTopology: true })
  .then((x) => {
  ...
  .then(() => {
    console.log('Attempting to insert the following movies:', movies);
    return Movie.insertMany(movies);
  })
  .then((insertedMovies) => {
    console.log('Inserted the following movies:', insertedMovies);
